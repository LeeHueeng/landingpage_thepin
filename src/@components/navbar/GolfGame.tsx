...
export default function GolfGame({ isOpen, onClose }: GolfGameProps) {
  ...
  const resetLevel = () => {
    ...
    const levelConfigs = [
      ...
      { ballX: 80, ballY: 150, holeX: 320, holeY: 250, obstacles: [{ x: 200, y: 180, width: 20, height: 80 }] },
      { ballX: 60, ballY: 100, holeX: 340, holeY: 280, obstacles: [
  ...
  const gameLoop = () => {
    ...
    if (!isDragging) {
      ...
      // 홀에 들어갔는지 확인 (더 관대한 조건)
      const distance = Math.sqrt((ball.x - hole.x) ** 2 + (ball.y - hole.y) ** 2)
      if (distance < hole.radius + ball.radius) {
        // 속도가 충분히 느려졌을 때만 홀에 들어감
        if (Math.abs(ball.velocityX) < 1.0 && Math.abs(ball.velocityY) < 1.0) {
          // 다음 레벨로
          if (currentLevel < 3) {
  ...
  const handleMouseUp = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    const ball = ballRef.current
    const power = Math.sqrt((x - dragStart.x) ** 2 + (y - dragStart.y) ** 2) * 0.1
